...
public class CropOverlayView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float cx = (EdgeType.LEFT + EdgeType.RIGHT)/2; 
        float cy = (EdgeType.TOP + EdgeType.BOTTOM)/2;
        float radius = (EdgeType.RIGHT - EdgeType.LEFT)/2;
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (mFixAspectRatio) {
            ...
            if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
                ...
                EdgeType.TOP = bitmapRect.top;
                EdgeType.BOTTOM = bitmapRect.bottom;

                final float centerX = getWidth() / 2f;

                // Limits the aspect ratio to no less than 40 wide or 40 tall
                final float cropWidth = Math.max(Edge.MIN_CROP_LENGTH_PX,
                                                 AspectRatioUtil.calculateWidth(EdgeType.TOP,
                                                                                EdgeType.BOTTOM,
                                                                                mTargetAspectRatio));
                ...
                if (cropWidth == Edge.MIN_CROP_LENGTH_PX)
                    mTargetAspectRatio = (Edge.MIN_CROP_LENGTH_PX) / (EdgeType.BOTTOM - EdgeType.TOP);

                final float halfCropWidth = cropWidth / 2f;
                EdgeType.LEFT = (centerX - halfCropWidth);
                EdgeType.RIGHT = (centerX + halfCropWidth);

            } else {

            	EdgeType.LEFT = bitmapRect.left;
            	EdgeType.RIGHT = bitmapRect.right;

                final float centerY = getHeight() / 2f;

                // Limits the aspect ratio to no less than 40 wide or 40 tall
                final float cropHeight = Math.max(Edge.MIN_CROP_LENGTH_PX,
                                                  AspectRatioUtil.calculateHeight(EdgeType.LEFT, EdgeType.RIGHT, mTargetAspectRatio));

                // Create new TargetAspectRatio if the original one does not fit
                // the screen
                if (cropHeight == Edge.MIN_CROP_LENGTH_PX)
                    mTargetAspectRatio = (EdgeType.RIGHT - EdgeType.LEFT) / Edge.MIN_CROP_LENGTH_PX;

                final float halfCropHeight = cropHeight / 2f;
                EdgeType.TOP = centerY - halfCropHeight;
                EdgeType.BOTTOM = centerY + halfCropHeight;
            }
            ...
            EdgeType.LEFT  = (bitmapRect.left + horizontalPadding);
            EdgeType.TOP = (bitmapRect.top + verticalPadding);
            EdgeType.RIGHT = (bitmapRect.right - horizontalPadding);
            EdgeType.BOTTOM = (bitmapRect.bottom - verticalPadding);
        }
    }
    ...
    public static boolean showGuidelines() {
        if ((Math.abs(EdgeType.LEFT - EdgeType.RIGHT) < DEFAULT_SHOW_GUIDELINES_LIMIT)
            || (Math.abs(EdgeType.TOP - EdgeType.BOTTOM) < DEFAULT_SHOW_GUIDELINES_LIMIT))
            return false;
        else
            return true;
    }
